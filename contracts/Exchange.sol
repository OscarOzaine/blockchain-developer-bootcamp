...
contract Exchange {
    ...
    function withdrawToken(address _token, uint256 _amount)
        public
    {
        require(tokens[_token][msg.sender] >= _amount, 'Address does not have sufficient balance');
        ...
        public
    {
        ...
        require(address(_order.user) == msg.sender, 'Caller must be owner of the order');
    ...
    function fillOrder(uint256 _id)
        public
    {
        require(_id > 0 && _id <= orderCount, 'Order id is not valid');

        require(!orderCancelled[_id], 'Order must not be cancelled');

        require(!orderFilled[_id], 'Order must not be filled');
