...
describe('Exchange', () => {
    let exchange, token1, token2, user1, user2, deployer, feeAccount;
    const feePercent = 10;
    ...
    describe('Depositing Tokens', () => {
        ...
        describe('Failure', () => {
            it('fails when no tokens are approved', async () => {
                await expect(exchange.connect(deployer).depositToken(token1.address, amount)).to.be
                .revertedWith('Address has to allow to transfer on their behalf');
            });
        });
    });
    ...
    describe('Withdrawing Tokens', () => {
        ...
        describe('Failure', () => {
            it('fails for insufficient balance', async () => {
                await expect(exchange.connect(deployer).withdrawToken(token1.address, amount)).to.be
                .revertedWith('Address does not have sufficient balance');
            });
        });
    });
            ...
            describe('Failure', () => {
                ...
                it('Rejects invalid order ids', async () => {
                    await expect(exchange.connect(user1).cancelOrder(9999)).to.be
                        .revertedWith('Order must exist');
                });

                it('Rejects unauthorized cancelations', async () => {
                    await expect(exchange.connect(user2).cancelOrder(1)).to.be
                        .revertedWith('Caller must be owner of the order');
                });
            });
        });
            ...
            describe('Failure', async () => {
                it('Rejects invalid order ids', async () => {
                    ...
                    await expect(exchange.connect(user2).fillOrder(invalidOrderid)).to.be
                        .revertedWith('Order id is not valid');
                });
                ...
                it('Rejects already filled orders', async () => {
                    ...
                    await expect(exchange.connect(user2).fillOrder(1)).to.be
                        .revertedWith('Order must not be filled');
                });
                ...
                it('Rejects canceled orders', async () => {
                    ...
                    await expect(exchange.connect(user2).fillOrder(1)).to.be
                        .revertedWith('Order must not be cancelled');
                });
            });
        });
    });
